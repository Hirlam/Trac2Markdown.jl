```@meta
EditURL="https://hirlam.org/trac//wiki//PlatformConfiguration?action=edit"
```
...
## Basic requirements
All experiments require a valid host to "setup" an experiment using the Harmonie script. Recall from the [quick start](./QuickStartLocal.md) instructions that in order to setup a new experiment on your platform, called YOURHOST, using HARMONIE downloaded to PATH_TO_HARMONIE one must issue the following command:
```bash
cd hm_home/my_exp
...
More information on Makeup is available here: [Build with Makeup](./Build_with_makeup.md)
### Obsmon
