...
The final namelists are build through the rules given in [`Get_namelist`](Harmonie/scr/Get_namelist?rev# release-43h2.beta.3) and are generated by [`gen_namelists.pl`](Harmonie/nam/gen_namelists.pl?revrelease-43h2.beta.3). Note that in several cases environment variables are still parsed in the scripts.
